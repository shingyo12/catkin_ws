...
double axis_rotangle = 0;

double AOV[32];
int    idx[32] = {0};

double cld_flxang = Flexion_Angle;
...
double delay_ang=-6;//1.67;
void messageCallBack00(const std_msgs::Float64& msg){
	//std::printf(" callback\n");
	//swing_cycle=msg.data;
	axis_rotangle = 90.0 + delay_ang;
	//delay_ang=delay_ang+1;
	axis_d_rotangle=0.1*(0.2/360)*(360/msg.data);
	//ROS_INFO_STREAM(axis_d_rotangle);
	std::printf("callback %lf %lf\n",axis_rotangle,axis_d_rotangle);
}

void messageCallBack01(const std_msgs::Float64& ang_msg){
	//std::printf(" callback\n");
	cld_flxang = Flexion_Angle + ang_msg.data;
	//ROS_INFO_STREAM(cld_flxang);
	std::cout<<cld_flxang<<"  "<<ang_msg.data<<std::endl;
}
...
void swing_coodinate_trans(struct dots *q, int pnum_start, int pnum_end){
	...
	double b = cld_flxang * M_PI/180;  //flexion angle of axis (rad)
	double d_lo = 24 + 37.7;  //distance : flexion point to laser origin (mm)
	//i is Block loop val.
	for(i = pnum_start; i < pnum_end; i++){
		...
		for(j = 0; j < 16; j++){
			...
			double ys = q->tY[i][j];
			double zs = q->tZ[i][j];
			q->tX[i][j] = (sin(a)*sin(a)+cos(a)*cos(a))*xs + b*cos(a)*zs + d_lo*b*cos(a);
			q->tY[i][j] = (sin(a)*sin(a)+cos(a)*cos(a))*ys + b*sin(a)*zs + d_lo*b*sin(a);
			q->tZ[i][j] = -b*cos(a)*xs -b*cos(a)*ys + zs + d_lo + Z_offset;
		}
	}
	//std::cout<<"ct size "<<size-2<<endl;
}

/*void pub_row_data(struct dots *q,double rad){
	pcl::PointCloud<pcl::PointXYZI> row_points;
	row_points.width=24;
   	row_points.height=16;
   	row_points.is_dense=true;
   	row_points.points.resize(row_points.width*row_points.height);
	for(int i = 0; i < 24; i++){
		for(int j = 0; j < 16; j++){
			row_points.at(i,j).x = q->tX[i][j];
			row_points.at(i,j).y = q->tY[i][j];
			row_points.at(i,j).z = q->tZ[i][j];
			row_points.at(i,j).intensity = q->tIntensity[i][j];
		}
	}
}*/
...
void receiveLRF(void *arg){
	...
	ros::NodeHandle nh;
	ros::Subscriber sub0 = nh.subscribe("cycle",1,messageCallBack00);
	ros::Subscriber sub1 = nh.subscribe("calib_flxang",1,&messageCallBack01);
	ros::Publisher pub  = nh.advertise<sensor_msgs::PointCloud2>("point2",10);
	ros::Publisher pub1  = nh.advertise<sensor_msgs::PointCloud2>("row_points",10);
