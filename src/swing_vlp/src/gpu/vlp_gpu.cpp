...
#include "swing_coodinate_trans.h"
...
void swing_coodinate_trans(struct dots *q, int pnum_start, int pnum_end, double rotangle, double d_rotangle){
	...
	float d_x[n*16],d_y[n*16],d_z[n*16];
	...
	swing_move(d_x,d_y,d_z,n,b,rotangle,d_rotangle,imu.y,imu.p,imu.r);
	for(i = pnum_start; i < pnum_end; i++){
		k++;
		for(j = 0; j < 16; j++){
			q->tX[i][j] = d_x[k*j];
...
void coodinate_trans(struct ppLRF *p, struct dots *q, const int dn, int& r){
	...
	swing_coodinate_trans( q, pnum_start, pnum_end, axis_rotangle, axis_d_rotangle);
}
