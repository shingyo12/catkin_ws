...
void receiveLRF(void *arg){
	...
	ros::NodeHandle nh;
	ros::Subscriber sub1 = nh.subscribe("cycle",1,&messageCallBack);
	ros::Subscriber sub2 = nh.subscribe("gps",1,&messageCallBack);
	...
	while(ros::ok()){
		...
		if(rtflg==1){
			...
			for(int r=0;r<dn*2-2;++r){
	    			for(int k=0;k<16;k++){	 
					trcloud.at(r,k).x=q.tX[r][k]+gps.position.x*1000;
		 			trcloud.at(r,k).y=q.tY[r][k]+gps.position.y*1000;
