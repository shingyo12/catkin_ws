...
int main(int argc, char** argv){
	...
	*filtered_cloud=voxel_grid(input_cloud);
	...
	cm_x=filtered_cloud->points[0].x*10;
	cm_y=filtered_cloud->points[0].y*10;
	max_x=min_x=(int)cm_x;
	max_y=min_y=(int)cm_y;
	for(int i=0; i<filtered_cloud->points.size(); i++){
		if(filtered_cloud->points[i].z*10>1 ){
			cm_x=filtered_cloud->points[i].x*10;
			cm_y=filtered_cloud->points[i].y*10;
			cm_z=filtered_cloud->points[i].z*10;
			grid_x=(int)cm_x;
			grid_y=(int)cm_y;
			grid_z_max=(int)cm_z;
			grid_z_min=(int)cm_z;
			for(int k=i+1; k<filtered_cloud->points.size(); k++){
				if(filtered_cloud->points[k].z*0.01>1 ){
					cm_x=filtered_cloud->points[k].x*10;
					cm_y=filtered_cloud->points[k].y*10;
					cm_z=filtered_cloud->points[k].z*10;
